...
void GaussianModel::scaledTransformVisiblePointsOfKeyframe(
    ...
    int& num_transformed,
    const Camera::CameraModelType camera_type,
    const float scale)
{
    torch::NoGradGuard no_grad;
...
void GaussianModel::removeTensorStateFromOptimizer(int tensor_idx)
{
    auto& param = this->optimizer_->param_groups()[tensor_idx].params()[0];
    auto& state = optimizer_->state();
    auto key = c10::guts::to_string(param.unsafeGetTensorImpl());
    ...
    bool prune_by_extent)
{
    ...
    if (max_screen_size) {
        torch::Tensor big_points_vs = this->max_radii2D_ > max_screen_size;
        torch::Tensor big_points_ws;
        if (prune_by_extent)
            big_points_ws = std::get<0>(this->getScalingActivation().max(/*dim=*/1)) > 0.1f * extent;
        else
            big_points_ws = torch::full(big_points_vs.sizes(), false, big_points_vs.options());
