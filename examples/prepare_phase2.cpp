...
int main(int argc, char** argv)
{
    ...
    if (device_type == torch::kCUDA)
        std::cout << "CUDA available! Preparing Phase 2 package on GPU." << std::endl;
    else
        std::cout << "Preparing Phase 2 package on CPU." << std::endl;
    ...
    mapper->exportPhase2FrozenPackage(output_dir);
